    ...
    stage('Rollback Action') {
        ...
        steps {
            script {
                ...
                def rollbackImage = 'retail-app:4.2.1'

                echo "Rollback image: ${rollbackImage}"

                bat """
                    "${DOCKER}" rm -f retail-app-production 2>nul
                ...
                bat """
                    ...
                    for (\$i=1; \$i -le 12; \$i++) { ^
                        ...
                        \$status = & '${DOCKER}' inspect --format='{{.State.Health.Status}}' retail-app-production 2>nul; ^
        ...
        steps {
            script {
                ...
                try {
                    ...
                    bat """
                        "${DOCKER}" rm -f ${candidateName} 2>nul
                    ...
                    bat """
                        ...
                        for (\$i=1; \$i -le 12; \$i++) { ^
                            Start-Sleep -Seconds 5; ^
                            \$status = & '${DOCKER}' inspect --format='{{.State.Health.Status}}' ${candidateName} 2>nul; ^
                    ...
                    bat """
                        ...
                        for (\$i=1; \$i -le 12; \$i++) { ^
                            Start-Sleep -Seconds 5; ^
                            \$status = & '${DOCKER}' inspect --format='{{.State.Health.Status}}' ${finalName} 2>nul; ^
