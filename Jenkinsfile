pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    env.IMAGE_TAG = "${params.VERSION}-${env.BUILD_NUMBER}"
            ...
            steps {
                script {
                    ...
                    bat """
                        ...
                        "${DOCKER}" run -d --name retail-app-production --network ${NETWORK_NAME} -p 8081:8081 --cpus="1.0" --memory="512m" -e APP_VERSION=4.2.1 -e APP_ENV=production -e PAYMENT_MODE=normal ${rollbackImage}
                    """

                    echo "Waiting for rollback health check..."

                    bat """
                        powershell -NoProfile -Command "\$healthy=\$false; for(\$i=1; \$i -le 12; \$i++){ Start-Sleep -Seconds 5; \$status=& '${DOCKER}' inspect --format='{{.State.Health.Status}}' retail-app-production 2>nul; Write-Host ('Rollback health attempt ' + \$i + ': ' + \$status); if(\$status -eq 'healthy'){\$healthy=\$true; break} }; if(-not \$healthy){ exit 1 }"
                    """
        ...
        stage('Deploy') {
            ...
            steps {
                script {
                    ...
                    def candidateName = "retail-app-candidate-${env.BUILD_NUMBER}"
                    ...
                    try {
                        ...
                        bat """
                            ...
                            "${DOCKER}" run -d --name ${candidateName} --network ${NETWORK_NAME} -p ${candidatePort}:8081 --cpus="1.0" --memory="512m" -e APP_VERSION=${params.VERSION} -e APP_ENV=${params.ENVIRONMENT} -e PAYMENT_MODE=normal ${IMAGE_REPOSITORY}:${IMAGE_TAG}
                        """

                        echo "Waiting for candidate health check..."

                        bat """
                            powershell -NoProfile -Command "\$healthy=\$false; for(\$i=1; \$i -le 12; \$i++){ Start-Sleep -Seconds 5; \$status=& '${DOCKER}' inspect --format='{{.State.Health.Status}}' ${candidateName} 2>nul; Write-Host ('Candidate health attempt ' + \$i + ': ' + \$status); if(\$status -eq 'healthy'){\$healthy=\$true; break}; if(\$status -eq 'unhealthy'){ break } }; if(-not \$healthy){ Write-Host 'Candidate health check FAILED'; exit 1 }"
                        """
                        ...
                        bat """
                            "${DOCKER}" run -d --name ${finalName} --network ${NETWORK_NAME} -p ${finalPort}:8081 --cpus="1.0" --memory="512m" -e APP_VERSION=${params.VERSION} -e APP_ENV=${params.ENVIRONMENT} -e PAYMENT_MODE=normal ${IMAGE_REPOSITORY}:${IMAGE_TAG}
                        """
                        ...
                        bat """
                            powershell -NoProfile -Command "\$healthy=\$false; for(\$i=1; \$i -le 12; \$i++){ Start-Sleep -Seconds 5; \$status=& '${DOCKER}' inspect --format='{{.State.Health.Status}}' ${finalName} 2>nul; Write-Host ('Final health attempt ' + \$i + ': ' + \$status); if(\$status -eq 'healthy'){\$healthy=\$true; break} }; if(-not \$healthy){ exit 1 }"
                        """
                        ...
                        if (params.ENVIRONMENT == 'PRODUCTION') {
                            ...
                            if (env.PREVIOUS_IMAGE &&
                                ...
                                bat """
                                    ...
                                    "${DOCKER}" run -d --name ${finalName} --network ${NETWORK_NAME} -p 8081:8081 --cpus="1.0" --memory="512m" -e APP_VERSION=4.2.1 -e APP_ENV=production -e PAYMENT_MODE=normal ${env.PREVIOUS_IMAGE}
                                """

                                echo "Waiting for rollback health check..."

                                bat """
                                    powershell -NoProfile -Command "\$healthy=\$false; for(\$i=1; \$i -le 12; \$i++){ Start-Sleep -Seconds 5; \$status=& '${DOCKER}' inspect --format='{{.State.Health.Status}}' ${finalName} 2>nul; Write-Host ('Rollback health attempt ' + \$i + ': ' + \$status); if(\$status -eq 'healthy'){\$healthy=\$true; break} }; if(-not \$healthy){ Write-Host 'Rollback health check FAILED'; exit 1 }"
                                """
        ...
        stage('Final Validation') {
            ...
            steps {
                script {
                    ...
                    bat """
                        powershell -NoProfile -Command "try { \$response=Invoke-WebRequest -UseBasicParsing http://localhost:${port}/version; Write-Host \$response.Content } catch { Write-Host 'Application validation failed'; exit 1 }"
                    """
                }
            }
        }
